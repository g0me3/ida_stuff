...
static dd_snd_script_parse(void) {
	...
	do {
		...
		if(cmd < 0x40) { 	// 00-3F
			...
			if(((cmd >= 0xC9)&&(cmd <= 0xCB))||(cmd == 0xD5)||(cmd == 0xD6)||(cmd == 0xD7)||(cmd == 0xDC)||(cmd == 0xDD)) {// no args
				ea = make_data_array(ea,1,dd_snd_script_opcode_name(cmd));
			} else if((cmd == 0xD3)) {// no args
				make_data_array(ea,1,dd_snd_script_opcode_name(cmd));
				stop = 1;
			} else if(((cmd >= 0xC0)&&(cmd <= 0xC8))||(cmd == 0xCC)||(cmd == 0xD4)||(cmd == 0xD8)||(cmd == 0xD9)||(cmd == 0xDA)) {	// 1 byte arg
				ea = make_data_array(ea,2,dd_snd_script_opcode_name(cmd));
			} else if((cmd == 0xDF)) {	// 2 byte arg
				ea = make_data_array(ea,3,dd_snd_script_opcode_name(cmd));
			} else if((cmd == 0xDB)||(cmd == 0xDE)) {	// 3 byte arg
				ea = make_data_array(ea,4,dd_snd_script_opcode_name(cmd));
			} else if(cmd == 0xCD) {	// 2 byte arg load with ret
				ea = make_data_array(ea,1,dd_snd_script_opcode_name(cmd));
//				jmp_ptr = Word(ea) + MK_FP(AskSelector(seg + 1), 0);
				ea = make_offset(ea,seg);
...
static dd_snd_lib_parse(void) {
	...
	if(ea != BADADDR) {
		...
		auto ptr_name = form("_apu_snd%04x",ptr_ea);
		MakeName(ptr_ea, ptr_name);
		ea = make_offset(ea, seg);
...
static main(void) {
	AddHotkey("Shift-O", "dd_snd_lib_parse");
	AddHotkey("Shift-L", "dd_snd_script_parse");
}
