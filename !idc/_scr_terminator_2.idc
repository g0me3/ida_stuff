...
//					if(ptr < 0xA000)	// terminator 2
//						base = MK_FP(AskSelector(0x9), 0);
//					else
//						base = MK_FP(AskSelector(0xA), 0);
					if(ptr < 0xC000)	// treasure master
						base = MK_FP(AskSelector(4+1), 0);
					else
...
static main(void) {
	AddHotkey("Shift-7", "term2_proto_apu_ch0");
	AddHotkey("Shift-6", "term2_proto_apu_ch3");
}
