...
// special non item objects
	if(val == 0x68) ret = "OBJ_68_vampire_trap"; else
	if(val == 0x6C) ret = "OBJ_6C_vampire_trap_base"; else
...
static scooby_get_obj_name(val) {
	...
	if(val == 0x08) ret = "OBJ08_Scooby"; else
...
static scooby_get_opc_name(opc) {
	...
	if(opc == 0x07) ret = "DELAY_RESET       "; else
	if(opc == 0x08) ret = "DELAY             "; else
	if(opc == 0x0D) ret = "MSGA              "; else
	if(opc == 0x0E) ret = "MSG_CLR           "; else
	if(opc == 0x0F) ret = "RANGE_TEST        "; else
	if(opc == 0x10) ret = "DOOR_LOCK_BNE     "; else
	if(opc == 0x11) ret = "WALK_TO_OBJ       "; else
	...
	if(opc == 0x30) ret = "DELAY_WITH_SKIP   "; else
	if(opc == 0x31) ret = "DOOR_LOCK         "; else
	if(opc == 0x32) ret = "DOOR_UNLOCK       "; else
...
//	if(opc == 0x36) ret = "ITEM_             "; else
	...
	if(opc == 0x4C) ret = "DOOR_SUB_SET      "; else
	if(opc == 0x4E) ret = "ITEM_DISABLE      "; else
...
static scooby_decompile_opcode(ea, ret) {
	...
	if(len != 0xFF) {
		...
		} else if((cmd == 0x07)||(cmd == 0x0E)||(cmd == 0x2A)||(cmd == 0x2F)||(cmd == 0x40)||(cmd == 0x41)||(cmd == 0x43)) {
			ea = scooby_make_opcode(ea, len, opcname, -1, -1, -1, -1);
		} else if((cmd == 0x31)) {
			ea = scooby_make_opcode(ea, len, opcname, ARG_NUM, ARG_NUM, ARG_ITM, -1);
		} else if((cmd == 0x33)) {
